...
alpha = 1e-4
    ...
    # Backward pass of the neural network
  backward <- function(data, label, params) {
      z <- forwardPropogate(batch_data, params)

      z1 <- z[[1]]
      z2 <- z[[2]]
      
      db2_temp <- batch_label - z2
      params$b2 <- colSums(db2_temp)
      params$W2 <- t(z1) %*% db2_temp
      db1_temp <- db2_temp %*% t(params$W2)
      db1_temp[z1 <= 0] <- 0
      params$b1 <- colSums(db1_temp)
      params$W1 <- t(data) %*% db1_temp
  ...
  forwardPropogate <-  function( input_data,params) {
      ...
      a1 <-  (input_data %*% params$W1) + params$b1
      params$z1 <- relu(a1)
      a2 <- (params$z1 %*% params$W2) + params$b2
      params$z2 <- softmax(a2)
...
l <- seq(1, length(y))
      ...
      for (epoch in 1:2) { #Iterate through the requested number of iterations
